...
pytest --cov=app --cov-report=json > ../analysis/backend-coverage.json
pip-audit --format json > ../analysis/backend-security.json

# Frontend
cd ../frontend
npm run lint -- --format json > ../analysis/lint-results.json
npm test -- --coverage --json > ../analysis/frontend-coverage.json
npm audit --json > ../analysis/frontend-security.json
```
